...
public class MainActivity extends Activity {
	private static final String TAG = "MusicFolderPlayer.MainActivity";
	...
	private ServiceConnection mServiceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			mService = ((MusicFolderPlayerService.MyBinder) service)
					.getService();
			startService(new Intent(MainActivity.this,
					MusicFolderPlayerService.class));
	...
	private void setPlayStopBtnVisible() {
		this.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				mBtnPlay.setVisibility((mIsPlaying) ? View.INVISIBLE
						: View.VISIBLE);
				mBtnStop.setVisibility((mIsPlaying) ? View.VISIBLE
						: View.INVISIBLE);
			}
		});
	}
	...
	private class MyBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (!MusicFolderPlayerService.INTENT_ACTION.equals(intent
					.getAction())) {
				return;
			}
			String notify = intent
					.getStringExtra(MusicFolderPlayerService.EXTRA_NOTIFY);
			if (notify.equals(MusicFolderPlayerService.NOTIFY_POSITION)) {
				int c = intent.getIntExtra(
						MusicFolderPlayerService.EXTRA_CURRENT, 0);
				int d = intent.getIntExtra(
						MusicFolderPlayerService.EXTRA_DURATION, 0);
				setTimeText(c, d);
			} else if (notify.equals(MusicFolderPlayerService.NOTIFY_PREPARE)) {
				final String path = intent
						.getStringExtra(MusicFolderPlayerService.EXTRA_PATH);
			...
			mThis.runOnUiThread(new Runnable() {
				...
				public void run() {
					if (d <= 0) {
						...
						mTxtDuration.setText(String.format("%3d:%02d", d / 60,
								d % 60));
						mTxtCurTime.setText(String.format("%3d:%02d", c / 60,
								c % 60));
					}
					mSeekBar.setMax(d);
	...
	private void setPlayingSongText(final String path) {
		...
		int i = path.lastIndexOf('/');
		final String album = path
				.substring(path.lastIndexOf('/', i - 1) + 1, i);
